    ...
    for (var iday = datelotterys.length - 1 - 32 - 2000; iday >= 0; iday--) {
        ...
        resultlotterybydate.forEach(function (element, index) {
            ...
            if (element.NameLottery === "G11") {
                ...
                profit_g11_3d_5days += (!_64s_g1[0].includes(g11_2) && _3ds.includes(parseInt(element.Value[2])) ? (972.5 - (100 - _64s_g1[0].length) * _3ds.length) * start_point : -1 * (100 - _64s_g1[0].length) * _3ds.length * start_point);
...
d3.json("https://raw.githubusercontent.com/bienhuynh/XSMBView/main/data/resultlotterytable_09.json", function (error, data) {
    if (error)
        throw error;
    data_global = data;
