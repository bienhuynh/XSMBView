...
var numberx2 = Oversem(LXXs.length) / (Oversem(4) * Oversem(LXXs.length - 4));
var startpoint = 10;
var invertings = [];
var oychart5 = [];
var oychart6 = [];
...
function createheadertable() {
    ...
    for (var i = 0; i < numberx2; i++) {
        htmlheader += '<th scope="col">X4_' + (i + 1) + '</th>';
    }
...
function mainstatics(data) {
    numberx2 = Oversem(LXXs.length) / (Oversem(4) * Oversem(LXXs.length - 4));
    createheadertable();
                ...
                for (var k = j + 1; k < f_end; k++) {
                    for (var f = k + 1; f < f_end; f++) {
                        ...
                        var x3 = (loto1.IsTrue ? '(' + loto1.LotoValue + ')' : loto1.LotoValue) + ' '
                            ...
                            + (loto4.IsTrue ? '(' + loto4.LotoValue + ')' : loto4.LotoValue);
                        rl_html += '<td class="text-center text-loterry ' + (loto1.IsTrue && loto2.IsTrue && loto3.IsTrue && loto4.IsTrue ? "text-true" : "text-fail") + '">' + x3 + '</td>';
                        x3elment.x3List.push({
                            loto1: loto1,
                            loto2: loto2,
                            loto3: loto3,
                            loto4: loto4,
                            isTrue: loto1.IsTrue && loto2.IsTrue && loto3.IsTrue && loto4.IsTrue
                        });
                    }
                }
            }
        ...
        for (var i = f_start; i < f_end; i++) {
            for (j = i + 1; j < f_end; j++) {
                for (var g = j + 1; g < f_end; g++) {
                    for (var f = g + 1; f < f_end; f++) {
                        var loto1 = element.LotoPredicts[LXXs[i] - 1];
                        var loto2 = element.LotoPredicts[LXXs[j] - 1];
                        var loto3 = element.LotoPredicts[LXXs[g] - 1];
                        var loto4 = element.LotoPredicts[LXXs[f] - 1];
                        if (loto1.IsTrue && loto2.IsTrue && loto3.IsTrue && loto4.IsTrue) {
                            profit += 2510 * basebet[lostday];
                            profitlinear += 251 * start_bet_linear;
                            revenuebymonthchart7 += 251 * start_bet_linear;
                        }
                    }
                }
            }
