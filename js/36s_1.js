...
d3.json("https://raw.githubusercontent.com/bienhuynh/XSMBView/main/data/resultlotterytable.json", function (error, data) {
    if (error)
        throw error;
    var _64s = [];
    var investday = -(100 - 64) * start_point;
    ...
    for (var iday = datelotterys.length - 1 - 32 - 0; iday >= 0; iday--) {
        ...
        resultlotterybydate.forEach(function (element, index) {
            if (element.NameLottery === "G00") {
                ...
                profit_g00_3d_5days += (!_64s[0].includes(g00_2) ? (972.5 - (100 - _64s[0].length) * _3ds.length) * start_point : -1 * (100 - _64s[0].length) * _3ds.length * start_point);
            ...
            if (element.NameLottery === "G11") {
                ...
                profit_g11_3d_5days += (!_64s[0].includes(g11_2) ? (972.5 - (100 - _64s[0].length) * _3ds.length) * start_point : -1 * (100 - _64s[0].length) * _3ds.length * start_point);
